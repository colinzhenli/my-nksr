...
class KernelField(BaseField):
    ...
    def evaluate_kernel(self, xyz: torch.Tensor, depth: int, grad: bool = False):
        ...
        if self.interpolator is not None:
            ...
            # if do_compute_grad:
            #     interp_res[0].register_hook(self.gradient_hook('interp_res'))
    ...
    def gradient_hook(self, name):
        def hook(grad):
            print(f"Gradients on {name}:", grad)
        return hook

    def evaluate_f_depth(self, xyz: torch.Tensor, depth: int, grad: bool = False):
        # if grad:
            # Attach the hook to the xyz tensor
            # xyz.register_hook(self.gradient_hook('xyz'))
        grid = self.svh.grids[depth]
        if grid is None:
        ...
        # f_depth.register_hook(self.gradient_hook('f_depth'))
        return EvaluationResult(f_depth, f_depth_grad if grad else None)

    def evaluate_f(self, xyz: torch.Tensor, grad: bool = True):
        f = EvaluationResult.zero(grad)
        for d in range(self.svh.depth):
            f_depth = self.evaluate_f_depth(xyz, d, grad)
            # xyz.register_hook(self.gradient_hook('f_depth'))
            f = f + f_depth
        return f
