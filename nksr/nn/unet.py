...
""" without voxel growing """
class NoGrowSparseStructureNet(nn.Module): 
    def __init__(self,
                 in_channels: int,
                 num_blocks: int,
                 basis_channels: int,
    ...
    def forward(self,
        ...
        for module, upsampler, struct_conv, normal_conv, basis_conv, udf_conv in \
            ...
            dec_coords = decoder_tmp_svh.grids[feat_depth].active_grid_coords()
            # decoder_svh.build_from_grid_coords(feat_depth, dec_coords)
            dec_main_feature, _, _ = self.padding(
                dec_main_feature, decoder_tmp_svh, feat_depth, decoder_svh
            )
            upsample_mask, _, _ = self.padding(
                (decoder_tmp_svh.grids[feat_depth].active_grid_coords()[:, 0])!=False, decoder_tmp_svh, feat_depth, decoder_svh
            )
